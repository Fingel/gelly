...
impl TopPage for PlaylistList {
    ...
    fn current_sort_by(&self) -> u32 {
        config::get_playlists_sort_by().min(self.sort_options().len() as u32 - 1)
    }
    ...
    fn setup_model(&self) {
        ...
        factory.connect_setup(glib::clone!(
            ...
            move |_, list_item| {
                ...
                let widget = Playlist::new();
                let binding = playlist_list
                    .bind_property(
                        "compact-mode",
        ...
        factory.connect_teardown(move |_, list_item| {
            let list_item = list_item.downcast_ref::<gtk::ListItem>().unwrap();
            if let Some(w) = list_item.child().and_downcast::<Playlist>() {
...
mod imp {
    ...
    #[properties(wrapper_type = super::PlaylistList)]
